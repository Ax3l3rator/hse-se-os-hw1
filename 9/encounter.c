...
int main(int argc, char* argv[]) {
    ...
    if ((file_write_descriptor = open(argv[2], O_WRONLY | O_APPEND | O_CREAT)) < 0) {
    ...
    while ((file_read_size = read(file_read_descriptor, buffer, buffer_size)) > 0) {
    ...
    fifo_read_size = read(fifo_read_descriptor, answer, buffer_size);
    // printf("%12s %d", answer, fifo_read_size);
    if (fifo_read_size >= 12 && strncmp(answer, "[no_answer]", 12) == 0) {
        truncate(argv[2], 0);
    }
